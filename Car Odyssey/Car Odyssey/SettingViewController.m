...
#define guideAPI @"http://115.29.246.88:9999/center/guide"
#define checkNewAPI @"http://115.29.246.88:9999/center/version"
...
- (void)configUI
{
    ...
    UITapGestureRecognizer *tap5 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(contactService)];
    NSArray *TapArr = @[tap,tap1,tap2,tap3,tap4,tap5];
    NSArray *titleArr = @[@"音效开关",@"关于车漫行",@"查看司机接单指南",@"检查新版本",@"意见反馈",@"联系客服"];
    ...
    for (int i = 0 ; i < 6; i ++) {
        ...
        UIView *view5 = [self.view viewWithTag:105];
        ...
        [outLog addTarget:self action:@selector(exitLog) forControlEvents:UIControlEventTouchUpInside];
...
- (void)checkNew
{
    ...
    [[NetManager shareManager]requestUrlPost:checkNewAPI andParameter:paraDic withSuccessBlock:^(id data)
    {
        if ([data[@"status"]isEqualToString:@"9000"])
        {
            ...
            {
                RedView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREENW, 64)];
                ...
                nikeM.image = [UIImage imageNamed:@"meiyouxinbanben"];
                ...
                textL.font = [UIFont systemFontOfSize:15*SCREENW_RATE];
                textL.text = @"没有新版本";
