...
#define withDrawDepoistAPi @"http://115.29.246.88:9999/center/extract"
...
- (void)configUI
{
    ...
    moneyTF = [[UITextField alloc]initWithFrame:CGRectMake(CGRectGetMaxX(moneyL.frame)+10*SCREENW_RATE, 0, 280*SCREENW_RATE, 50*SCREENW_RATE)];
    moneyTF.delegate = self;
    moneyTF.font = [UIFont systemFontOfSize:16];
    [moneyTF setAttributedPlaceholder:[[NSAttributedString alloc]initWithString:@"建议输入整数金额" attributes:@{NSForegroundColorAttributeName:RGB(206, 206, 206)}]];
    [moneyV addSubview:moneyTF];
    
    UIView *accountV = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(moneyV.frame)+10*SCREENW_RATE, SCREENW, 50*SCREENW_RATE)];
    accountV.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:accountV];
    
    UILabel *accountL = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 60*SCREENW_RATE, 50*SCREENW_RATE)];
    accountL.textAlignment = NSTextAlignmentCenter;
    accountL.textColor = RGB(51, 51, 51);
    accountL.font = [UIFont systemFontOfSize:16];
    accountL.text = @"账户";
    [accountV addSubview:accountL];
    
    accountTF = [[UITextField alloc]initWithFrame:CGRectMake(CGRectGetMaxX(accountL.frame)+10*SCREENW_RATE, 0, 280*SCREENW_RATE, 50*SCREENW_RATE)];
    accountTF.font = [UIFont systemFontOfSize:16];
    [accountTF setAttributedPlaceholder:[[NSAttributedString alloc]initWithString:@"请输入您的账户" attributes:@{NSForegroundColorAttributeName:RGB(206, 206, 206)}]];
    [accountV addSubview:accountTF];
    
    UIButton *sureBtn = [[UIButton alloc]initWithFrame:CGRectMake(15*SCREENW_RATE, CGRectGetMaxY(accountV.frame)+36*SCREENW_RATE, 345*SCREENW_RATE, 50*SCREENW_RATE)];
    ...
    [sureBtn addTarget:self action:@selector(withdrawDeposit:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:sureBtn];
    
    NSString *str = @"提现完成后可以在";
...
- (void)withdrawDeposit:(UIButton *)selectedBtn
{
    ...
    [paraDic setObject:moneyTF.text forKey:@"money"];
    [paraDic setObject:accountTF.text forKey:@"account"];
    [paraDic setObject:name forKey:@"name"];
    if (chooseZFB == NO)
    {
        [paraDic setObject:@"0" forKey:@"channel"];
    }
    else
    {
        [paraDic setObject:@"1" forKey:@"channel"];
    }
    [[NetManager shareManager]requestUrlPost:withDrawDepoistAPi andParameter:paraDic withSuccessBlock:^(id data)
    {
